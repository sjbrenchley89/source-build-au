...
const FEATURED = [
  { id: 1, title: 'Cabinetry & Joinery', tag: 'Custom Supply', colSpan: 'md:col-span-7', aspect: 'aspect-[4/3]', img: 'https://images.unsplash.com/photo-1556909114-f6e7ad7d3136?w=800&q=80' },
  { id: 2, title: 'Windows & Aluminium Systems', tag: 'Glazing & Facades', colSpan: 'md:col-span-5', aspect: 'aspect-[4/3]', img: 'https://images.unsplash.com/photo-1497366216548-37526070297c?w=800&q=80' },
  { id: 3, title: 'Stone Benchtops & Vanities', tag: 'Surfaces', colSpan: 'md:col-span-5', aspect: 'aspect-[4/3]', img: 'https://images.unsplash.com/photo-1600607686527-6fb886090705?w=800&q=80' },
  { id: 4, title: 'Bathrooms & Wet Areas', tag: 'Sanitaryware', colSpan: 'md:col-span-7', aspect: 'aspect-[4/3]', img: 'https://images.unsplash.com/photo-1552321554-5fefe8c9ef14?w=800&q=80' },
]
        ...
        <motion.div className="flex items-start justify-between mb-10"
          ...
          <div>
            ...
            <p className="text-sm text-muted max-w-sm">
              Factory-direct building materials sourced globally and delivered to your project site — anywhere in Australia.
            </p>
          </div>
          <GradientBorderButton className="hidden md:inline-flex" href="#contact">Get a Proposal &#8594;</GradientBorderButton>
        </motion.div>
        ...
        <motion.div variants={fadeUp} initial="hidden" whileInView="visible" viewport={{ once: true, margin: '-30px' }}>
          ...
          <div className="flex flex-wrap gap-2">
            {ALL_CATEGORIES.map(cat => (
              <span key={cat} className="text-xs text-muted border border-stroke rounded-full px-4 py-2 hover:text-text-primary hover:border-[#F26522]/50 transition-colors duration-200 cursor-default">
                {cat}
              </span>
            ))}
          </div>
  ...
  return (
    <div className={`group relative overflow-hidden rounded-3xl border border-stroke bg-surface cursor-pointer ${product.aspect}`}>
      ...
      <div className="absolute top-4 left-4">
        <span className="text-xs text-muted bg-bg/80 backdrop-blur-sm rounded-full px-3 py-1 border border-stroke">{product.tag}</span>
      </div>
    </div>
  )
}
...
function GradientBorderButton({ children, href, className = '' }: { children: React.ReactNode; href: string; className?: string }) {
  return (
    <div className={`relative group ${className}`}>
      ...
      <a href={href} className="relative z-10 inline-flex items-center gap-2 rounded-full border border-stroke bg-bg text-sm text-muted hover:text-text-primary px-5 py-2 transition-colors duration-200">{children}</a>
    </div>
  )
}

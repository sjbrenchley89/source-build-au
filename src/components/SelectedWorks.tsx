        ...
        >
          <div>
            ...
            <p className="text-sm text-muted max-w-sm">
              Factory-direct building materials sourced globally and delivered to your project site across Melbourne and Australia.
            </p>
          </div>
...
function ProductCard({ product }: { product: (typeof PRODUCTS)[0] }) {
  return (
    <div
      className={`group relative overflow-hidden rounded-3xl border border-stroke bg-surface cursor-pointer ${product.aspect}`}
    >
      <img
        src={product.img}
        alt={product.title}
      ...
      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
        <div className="relative">
          ...
          <span className="relative flex items-center gap-2 bg-white text-bg rounded-full px-5 py-2 text-sm font-medium">
            Explore — <em className="font-display italic">{product.title}</em>
          </span>
        </div>
      </div>

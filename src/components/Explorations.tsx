...
export default function Explorations() {
  ...
  useEffect(() => {
    ...
    const pin = ScrollTrigger.create({ trigger: content, start: 'top top', end: () => `+=${section.offsetHeight - window.innerHeight}`, pin: true, pinSpacing: false })
      ...
      <div ref={contentRef} className="relative z-10 h-screen flex items-center justify-center">
        <div className="text-center px-6 max-w-lg">
          ...
          <p className="text-sm text-muted mb-8">
            From residential builders to civil contractors — if your project needs building materials, we have the supply chain to back you.
          </p>
          <div className="relative group inline-flex">
            ...
            <a href="#contact" className="relative z-10 inline-flex items-center gap-2 rounded-full border border-stroke bg-bg text-sm text-muted hover:text-text-primary px-6 py-3 transition-colors duration-200">Get a Supply Proposal &#8599;</a>
          </div>
        </div>
      </div>
...
function ClientCard({ item }: { item: (typeof CLIENTS)[0] }) {
  return (
    <div className="relative aspect-square max-w-[320px] rounded-2xl overflow-hidden border border-stroke pointer-events-auto" style={{ transform: `rotate(${item.rotation}deg)` }}>
